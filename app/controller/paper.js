  ...
  async show() {
    ...
    if (paper) {
      ...
      if (!_.isEmpty(questions)) {
        const options = await service.questionOption.get({ question_id: questionIdArr });
        const qo = {};
        options.forEach(item => {
          if (!qo[item.question_id]) {
            qo[item.question_id] = [];
          }
          qo[item.question_id].push(item);
        });

        questions.forEach((item, index) => {
          questions[index].options = qo[item.id];
        });
      }
